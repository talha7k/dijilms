...
Users SHALL have roles including 'user', 'admin', 'moderator', 'teacher', 'student', 'parent', 'management'.
...
Existing users without a role SHALL be assigned 'user' role.

#### Scenario: Existing user profile lacks role

- **WHEN** an existing user logs in without a role set

- **THEN** their role is defaulted to 'user'
